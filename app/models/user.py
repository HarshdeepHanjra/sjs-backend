...
class Student(db.Model):
    ...
    course_ids = db.Column(db.JSON, default=[])
    certificate_count = db.Column(db.Integer, default=0)
    internship_ids = db.Column(db.JSON, default=[])
    ...
    # When a student is deleted, all related records will be automatically deleted
    internship_orders = db.relationship(
        'InternshipOrder', 
        backref='student_rel',
        cascade='all, delete-orphan',
        lazy=True,
        foreign_keys='InternshipOrder.student_id'
    )
    
    internship_enrollments = db.relationship(
        'InternshipEnrollment',
        backref='student_rel',
        cascade='all, delete-orphan',
        lazy=True,
        foreign_keys='InternshipEnrollment.student_id'
    )
    
    payment_verifications = db.relationship(
        'PaymentVerification',
        backref='student_rel',
        cascade='all, delete-orphan',
        lazy=True,
        foreign_keys='PaymentVerification.student_id'
    )
    ...
    def to_dict(self):
        return {
            ...
            'status': self.status,
            'course_ids': self.course_ids or [],
            'internship_ids': self.internship_ids or [],
            'certificate_count': self.certificate_count,
            'occupation': self.occupation or '',
            'company': self.company or '',
            'skills': self.skills or '',
