...
MAIL_USERNAME = os.getenv('MAIL_USERNAME', '')
MAIL_PASSWORD = os.getenv('MAIL_PASSWORD', '')
MAIL_DEFAULT_SENDER = os.getenv('MAIL_DEFAULT_SENDER', 'noreply@sjsglobaltech.com')  # ✅ Changed
BREVO_API_KEY = os.getenv('BREVO_API_KEY', '')
USE_REAL_EMAIL = os.getenv('USE_REAL_EMAIL', 'True').lower() == 'true'
ADMIN_EMAIL = os.getenv('ADMIN_EMAIL', 'gangahanjra@gmail.com')
...
def send_email_smtp(to_email, subject, html_content, text_content=None):
    ...
    try:
        ...
        # Create message
        msg = MIMEMultipart('alternative')
        msg['From'] = f"SJS Global Tech <{MAIL_DEFAULT_SENDER}>"  # ✅ Changed
        msg['To'] = to_email
        msg['Subject'] = subject
...
def send_admin_notification_email(student_name, student_email, order_id, amount, transaction_id, payment_method, courses, screenshot_url):
    ...
    # Email body (HTML) - Changed "SJS Global Tech Academy" to "SJS Global Tech"
    html_body = f"""
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <body>
        <div class="container">
            ...
            <div class="footer">
                <p>This is an automated notification from SJS Global Tech.</p>  <!-- ✅ Changed -->
                <p>© 2024 SJS Global Tech. All rights reserved.</p>  <!-- ✅ Changed -->
            </div>
        </div>
    </body>
    </html>
    ...
    # Plain text version
    text_body = f"""
    NEW PAYMENT VERIFICATION REQUEST - SJS Global Tech  # ✅ Changed
...
def send_student_confirmation_email(student_name, student_email, order_id, amount, courses):
    ...
    # ✅ Subject - Clear and welcoming
    subject = "✅ Payment Verified - Welcome to SJS Global Tech!"  # ✅ Changed
    ...
    <body>
        <div class="container">
            <div class="header">
                <h2>🎉 Payment Verified Successfully!</h2>
                <p style="margin: 5px 0 0 0;">Welcome to SJS Global Tech</p>  <!-- ✅ Changed -->
            </div>
            ...
            <div class="content">
                ...
                <p style="margin-top: 20px;">Thank you for choosing SJS Global Tech!</p>  <!-- ✅ Changed -->
                <p><strong>Best regards,</strong><br>SJS Global Tech Team</p>  <!-- ✅ Changed -->
            </div>
            
            <div class="footer">
                <p>This is an automated email from SJS Global Tech.</p>  <!-- ✅ Changed -->
                <p>© 2024 SJS Global Tech. All rights reserved.</p>  <!-- ✅ Changed -->
            </div>
        </div>
    </body>
    ...
    # ✅ Plain Text Version
    text_body = f"""
    PAYMENT VERIFIED - Welcome to SJS Global Tech!  # ✅ Changed
    ...
    Thank you for choosing SJS Global Tech!  # ✅ Changed
    
    Best regards,
    SJS Global Tech Team  # ✅ Changed
    """
...
def get_bank_details():
    ...
    bank_details = {
        'account_name': os.getenv('BANK_ACCOUNT_NAME', 'SJS Global Tech'),  # ✅ Changed
        'account_number': os.getenv('BANK_ACCOUNT_NUMBER', '123456789012'),
        'bank_name': os.getenv('BANK_NAME', 'State Bank of India'),
        'ifsc_code': os.getenv('BANK_IFSC_CODE', 'SBIN0012345'),
        'branch': os.getenv('BANK_BRANCH', 'Main Branch'),
...
def upload_screenshot():
    ...
    try:
        ...
        if not order_id:
            return cors_response({'error': 'Order ID is required'}, 400)
        
        if file.filename == '':
            return cors_response({'error': 'No file selected'}, 400)
        
        if not allowed_file(file.filename):
            return cors_response({'error': 'Invalid file type. Allowed: png, jpg, jpeg, gif, webp'}, 400)
...
def submit_verification():
    ...
    try:
        ...
        if not transaction_id:
            return cors_response({'error': 'Transaction ID is required'}, 400)
        
        if not screenshot_url:
            return cors_response({'error': 'Screenshot is required'}, 400)
        ...
        existing = PaymentVerification.query.filter_by(order_id=order_id).first()
        if existing:
            return cors_response({'error': 'Verification already submitted'}, 400)
...
def get_verification_status(verification_id):
    ...
    try:
        ...
        if not verification:
            return cors_response({'error': 'Verification not found'}, 404)
