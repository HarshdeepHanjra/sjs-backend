...
def cors_response(data, status=200, origin='https://sjsglobaltech.com'):
...
def change_password():
    if request.method == 'OPTIONS':
        ...
        response.headers.add('Access-Control-Allow-Origin', 'https://sjsglobaltech.com')
    ...
    try:
        ...
        if not current_password or not new_password:
            return cors_response({'error': 'Current password and new password are required'}, 400)
        ...
        if not hasattr(user, 'password_hash') or not user.password_hash:
            return cors_response({'error': 'Password reset not supported for this account type'}, 400)
        
        if not bcrypt.check_password_hash(user.password_hash, current_password):
            return cors_response({'error': 'Current password is incorrect'}, 401)
...
def reset_password():
    ...
    try:
        ...
        if not token or not new_password:
            return cors_response({'error': 'Token and new password are required'}, 400)
        ...
        if not token_data:
            return cors_response({'error': 'Invalid or expired reset token'}, 400)
        
        if token_data['expiry'] < datetime.utcnow():
            del reset_tokens[token]
            return cors_response({'error': 'Reset token has expired'}, 400)
        ...
        if not hasattr(user, 'password_hash'):
            return cors_response({'error': 'Password reset not supported for this account'}, 400)
...
def verify_reset_token():
    ...
    try:
        ...
        if not token:
            return cors_response({'valid': False, 'error': 'Token required'}, 400)
        
        token_data = reset_tokens.get(token)
        
        if token_data and token_data['expiry'] > datetime.utcnow():
            return cors_response({'valid': True}, 200)
        
        return cors_response({'valid': False, 'error': 'Invalid or expired token'}, 400)
        
    except Exception as e:
        print(f"Verify token error: {e}")
        return cors_response({'valid': False, 'error': str(e)}, 400)
...
def student_register():
    ...
    try:
        ...
        existing = Student.query.filter_by(email=email).first()
        if existing:
            return cors_response({'error': 'Email already registered'}, 400)
...
def student_login():
    ...
    try:
        ...
        if not student:
            print(f"Student not found: {email}")
            return cors_response({'error': 'Invalid email or password'}, 401)
        ...
        if email == 'admin@sjsacademy.com' and password == 'Admin@123':
            ...
            if not otp and not session_id:
                ...
                if success:
                    ...
                    return cors_response({'error': 'Failed to send OTP. Please try again.'}, 500)
            
            # Second step - Verify OTP
            elif otp and session_id:
                if not hasattr(current_app, 'login_sessions') or session_id not in current_app.login_sessions:
                    return cors_response({'error': 'Session expired. Please login again.'}, 401)
                ...
                if verify_otp(session_data['email'], otp):
                    ...
                    return cors_response({'error': 'Invalid or expired OTP'}, 401)
            else:
                return cors_response({'error': 'Invalid request'}, 400)
...
def resend_otp_code():
    ...
    try:
        ...
        if not session_id:
            return cors_response({'error': 'Session ID required'}, 400)
        ...
        if success:
            ...
            return cors_response({'error': 'Failed to resend OTP'}, 500)
...
def verify_auth_token():
    ...
    try:
        auth_header = request.headers.get('Authorization')
        if not auth_header:
            return cors_response({'valid': False, 'error': 'No token provided'}, 401)
        ...
        if payload:
            if payload.get('role') == 'student':
                user = Student.query.get(payload.get('id'))
                if not user:
                    return cors_response({'valid': False, 'error': 'User not found'}, 401)
            ...
            return cors_response({'valid': True, 'user': payload}, 200)
        else:
            return cors_response({'valid': False, 'error': 'Invalid token'}, 401)
            
    except Exception as e:
        print(f"Verify token error: {e}")
        return cors_response({'valid': False, 'error': str(e)}, 401)