...
# # # # Add these static methods to Certificate model if not exists
# # # # These should be in models/certificate.py but adding here for compatibility
...
# # # @certificates_bp.route('/api/certificates/generate', methods=['POST', 'OPTIONS'])
# # # @certificates_bp.route('/certificates/generate', methods=['POST', 'OPTIONS'])
# # # @admin_required
...
# # #         certificate_id = Certificate.generate_certificate_id(student_id)
# # #         verification_token = Certificate.generate_verification_token()
# # #         certificate_hash = Certificate.generate_certificate_hash(certificate_id, student.email, course.name)
        
# # #         # Generate QR Code with verification URL
# # #         verification_url = f"http://localhost:3000/verify-certificate?token={verification_token}"
...
# # # # Route to get student's certificates
# # # @certificates_bp.route('/api/certificates/my-certificates', methods=['GET', 'OPTIONS'])
# # # @certificates_bp.route('/certificates/my-certificates', methods=['GET', 'OPTIONS'])
# # # @token_required
...
# # # @certificates_bp.route('/api/certificates/verify', methods=['GET', 'OPTIONS'])
# # # @certificates_bp.route('/certificates/verify', methods=['GET', 'OPTIONS'])
...
# # #         # Try to find certificate by verification_token
...
# # #             print(f"Total certificates in DB: {len(all_certs)}")
# # #             for c in all_certs:
# # #                 print(f"  - Token: {c.verification_token[:20]}...")
...
# # # @certificates_bp.route('/api/certificates/download/<certificate_id>', methods=['GET', 'OPTIONS'])
# # # @certificates_bp.route('/certificates/download/<certificate_id>', methods=['GET', 'OPTIONS'])
# # # @token_required
...
# # # @certificates_bp.route('/api/admin/certificates', methods=['GET', 'OPTIONS'])
...
# # # @certificates_bp.route('/api/admin/certificates/<int:certificate_id>/revoke', methods=['POST', 'OPTIONS'])
...
# # # @certificates_bp.route('/api/certificates/debug/<token>', methods=['GET'])
# # # @certificates_bp.route('/certificates/debug/<token>', methods=['GET'])
# # # @admin_required
...
# # # Make sure the blueprint name is 'certificates_bp'
# # certificates_bp = Blueprint('certificates', __name__)
...
# #         # Generate certificate ID using the model method
...
# # @certificates_bp.route('/api/admin/certificates/<int:certificate_id>', methods=['DELETE', 'OPTIONS'])
...
# #             qr_path = os.path.join('uploads/certificates', os.path.basename(certificate.qr_code_url))
...
# #             pdf_path = os.path.join('uploads/certificates', os.path.basename(certificate.pdf_url))
...
# # Route to download certificate - SINGLE DEFINITION (removed duplicate)
# @certificates_bp.route('/api/certificates/download/<certificate_id>', methods=['GET', 'OPTIONS'])
# @token_required
# def api_download_certificate(certificate_id):
#     if request.method == 'OPTIONS':
...
# ✅ FIXED: Generate certificate ID without requiring student_id
def generate_certificate_id():
...
# ✅ FIXED: Generate verification token
def generate_verification_token():
    """Generate a unique verification token"""
    return secrets.token_hex(32)

# ✅ FIXED: Generate certificate hash
def generate_certificate_hash(certificate_id, student_email, course_name):
...
def api_generate_certificate():
    ...
    try:
        ...
        # ✅ FIXED: Use local functions instead of model methods
